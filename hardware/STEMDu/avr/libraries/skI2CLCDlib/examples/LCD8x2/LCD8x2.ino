...
skI2CLCDlib LCD(0x3E,8) ;              // ＬＣＤのアドレス、画面カラム数８文字
...
void setup()
{
     ...
     LCD.Init(0,32,1) ;                 // ICON OFF,コントラスト(0-63),VDD=3.3Vで使う

     LCD.CreateChar(1,heart) ;          // 1番にオリジナルキャラクタを登録する

     // ０～３ページ目に書き込む(０ページのみ表示される)
     ...
     LCD.Puts("0111111001111110") ;     // [40H]から書込まれる("01111110"を表示する)
     LCD.Puts("abcdefghijklmnop") ;     // [50H]から書込まれる(表示しない)
     delay(5000) ;
     // ２ページ目に切り替えて表示する
     LCD.PageSet(2) ;                   // 2ページ目の表示切り替え
     delay(5000) ;
     // ２ページ目を書き換える
     LCD.PageClear() ;                  // 2ページ目のクリア
     LCD.PageSetCursor(0,0) ;           // 2ページ目の位置でカーソル指示、[10H]に設定
     LCD.Puts("Arduino") ;              // 2ページ目[10H]から書込まれる(表示する)
     delay(5000) ;
     // ０ページ目に表示を切り替える
